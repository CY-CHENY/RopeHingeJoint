    ...
    public class RopeMesh : MonoBehaviour
    {
        ...
        public void CreateRopeMesh(Vector3[] points, float radius, int segmentsPerWire)
        {
            ...
            for (int i = 0; i < points.Length; i++)
            {
                ...
                float factor;//= 1 - Mathf.Abs(2 * t - 1);//中间为1 两端为0

                if (t < 0.1f)
                {
                    factor = Mathf.Lerp(0, 1, t / 0.1f);
                }
                else if (t > 0.9f)
                {
                    factor = Mathf.Lerp(1, 0, (t - 0.9f) / 0.1f);
                }
                else
                {
                    factor = 1;
                ...
                for (int j = 0; j <= segmentsPerWire; j++)
                {
                    ...
                    Vector3 offset = new Vector3(Mathf.Cos(angle), Mathf.Sin(angle), 0) * currentRadius; // * radius
            ...
            //生成起点端
            float startRadius = radius * (1 - Mathf.Abs(2 * 0f - 1));
            if (startRadius > 0)
            {
            ...
            //生成终点端盖
            float endT = 1f;
            float endRadius = radius * (1 - Mathf.Abs(2 * endT - 1));
            if (endRadius > 0)
            {
